...
Carnival_opt_SPN <-function(iterator_index,
                        df_EM,
                        results_dir,
                        inputs_dir,
                        disease_filename_j,
  ...
  for (ii in 1: length(tfList)){
    ...
    temp_dir <- shortPathName(paste0(carnival_path, "//", names(tfList)[ii]))
    print(temp_dir)
    ...
    setwd(temp_dir)
    print(temp_dir)
    dot_file <- shortPathName(paste0(temp_dir,"//network_solution.dot"))
  ...
  for (i in 1:  range_opt) {
    ...
    Sys.sleep(1 / total_Bar)
    net_base <- NULL
    temp_dir <- shortPathName(paste0(carnival_path, "//", names(tfList)[i]))
    base_file2 <- shortPathName(paste0(temp_dir,"//network_solution.dot"))
    base_file <- shortPathName(paste0(temp_dir,"//network_solution.graphml"))

    new_name_gml <- paste0(temp_dir,"//", as.character(names(tfList)[i]),".graphml")
    new_name_gml <- shortPathName(new_name_gml)

    if (file.exists(base_file) & (!file.exists(new_name_gml))){
    ...
    if (is.null(net_base) == FALSE) {
      ...
      try(
        visSave(sp_g_6, file = shortPathName(paste0(carnival_path, "/", names(tfList)[i],"/GRAPH",".html")), selfcontained = FALSE, background = "white")
      )
      ####################################################
      ...
      try(
        visSave(sp_g_5, file = shortPathName(paste0(carnival_path, "/", names(tfList)[i],"/CM",".html")), selfcontained = FALSE, background = "white")
      )
      print("Done")
  ...
  print(signatures)
  saveRDS(signatures, file="signatures.RData")
  ...
  #### RUN PYTHON SCRIPT TO CONVERT .dot to .graphml so that igraph can read it
  setwd(source_code_dir)
  carnival_path <- shortPathName(carnival_path)
