...
#CCLE_models<- models %>% dplyr::filter(OncotreeLineage == input$OncotreeLineage)
...
ui <- fluidPage(
  ...
  titlePanel("CCLE Data Analysis & Network Optimization 2024"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output,session) {
  ...
  observe({
    ...
    #shinyjs::toggleState("clinical_flag", input$CARNIVAL_flag == "TRUE" & input$molecular_flag == "TRUE")
    #shinyjs::toggleState("molecular_flag", input$CARNIVAL_flag == "TRUE"  & input$clinical_flag == "TRUE")
