...
def main(directory, csv_file, num_epochs, learning_rate, tts, min_obs, bsu, hidden_dim, dropout_prob, heads_user, hops_user, optuna, random_seed, goi_list):
    ...
    if subsample: 
      # --- Parameters ---
      max_per_class = 1000
      print(f"[INFO] Using subsampling...")
      
      # --- Original distribution ---
      original_dist = label_df["labels"].value_counts().sort_index()
      
      # --- Subsample ---
      subsampled_df = label_df.groupby("labels", group_keys=False).apply(
          lambda x: x.sample(n=min(len(x), max_per_class), random_state=42)
      ).reset_index(drop=True)
      
      # --- Subsampled distribution ---
      subsampled_dist = subsampled_df["labels"].value_counts().sort_index()
      
      # --- Save subsampled CSV ---
      subsampled_df.to_csv("labels_subsampled.csv", index=False)
      print(f"[INFO] Saved subsampled label file to labels_subsampled.csv with {len(subsampled_df)} rows.")
      ...
      #if not optuna:
      #  plt.show()
      print("[INFO] Saved class distribution pie charts to RESULTS/class_balance_pies.png")
      label_df = subsampled_df
            ...
            for file in os.listdir(folder_path):
                if file.endswith(".graphml"):
                    ...
                    if "fillcolor" in graph.vs.attributes():
                        ...
                        genes_of_interest =  goi_list
                        
                        missing_genes = [gene for gene in genes_of_interest if gene not in graph.vs["name"]]
                        #if missing_genes:
                        #    print(f"Missing GOI(s): {missing_genes} in graph {file}")
    ...
    # Standard 2-way split
      train_graphs, test_graphs, train_labels, test_labels = train_test_split(
        graphs, labels, test_size=1 - tts, stratify=labels, random_state=random_seed
      )
...
if __name__ == "__main__":
    ...
    parser.add_argument("repeat_exp", type=int, default=1, help="Number of repetitions with different splits")
    ...
    all_results = []  # stores tuples of (GOI, hop_order, run_id, misclf)

    os.makedirs("RESULTS", exist_ok=True)
    csv_path = "RESULTS/multi_scenario_results.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["GOI", "hop_order", "run", "random_seed", "misclassification_percent"])

    run_counter = 1
    for goi_set in goi_sets:
        for hop in hop_orders:
            for i in range(args.repeat_exp):
                random_seed = i + 1
                print(f"\n>>> RUN {run_counter} | GOI: {goi_set} | hop={hop} | seed={random_seed}")
                plt.ioff()
                misclf = main(
                    args.directory, args.csv_file, args.num_epochs, args.learning_rate,
                    args.tts, args.min_obs, args.bsu, args.hidden_dim, args.dropout_prob,
                    args.heads_user, hop, args.optuna, random_seed, goi_set  # <-- pass GOI
                )

                all_results.append((str(goi_set), hop, i + 1, random_seed, misclf))
                with open(csv_path, "a", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow([str(goi_set), hop, i + 1, random_seed, misclf])
                run_counter += 1
    ...
    df = pd.DataFrame(all_results, columns=["GOI", "hop_order", "run", "seed", "misclf"])
      
    plt.figure(figsize=(12, 8))
    sns.boxplot(data=df, x="GOI", y="misclf", hue="hop_order")
    plt.title("Misclassification % across GOI sets and Hop Orders")
    plt.ylabel("Misclassification (%)")
    plt.xlabel("Gene(s) of Interest (GOI)")
    plt.legend(title="Hop Order")
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig("RESULTS/master_misclassification_plot.png", dpi=600, bbox_inches='tight')
    plt.close()
