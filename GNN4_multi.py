...
def main(directory, csv_file, num_epochs, learning_rate, tts, min_obs, bsu, hidden_dim, dropout_prob, heads_user, hops_user, optuna, random_seed):
...
if __name__ == "__main__":
    ...
    import csv, random
    os.makedirs("RESULTS", exist_ok=True)
    csv_path = "RESULTS/seed_runs.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["run", "random_seed", "misclassification_percent"])

    for i in range(args.repeat_exp):
        random_seed = i
        print(f"\n===== RUN {i+1}/{args.repeat_exp} with random_seed = {random_seed} =====")
        ...
        misclf = main(
            ...
            args.heads_user, args.hops_user, args.optuna, random_seed
        )

        results.append((i + 1, random_seed, misclf))

        with open(csv_path, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([i + 1, random_seed, misclf])

    # Plot
    run_ids, seeds, misclfs = zip(*results)
    plt.figure()
    plt.plot(run_ids, misclfs, marker="o")
    plt.xlabel("Run #")
    plt.ylabel("Misclassification %")
    plt.title("Misclassification % vs Train/Test Split")
    plt.grid(True)
    plt.savefig("RESULTS/misclassification_runs.png", dpi=600)
    plt.close()
