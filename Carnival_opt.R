...
Carnival_opt <-function(iterator_index,
  ...
  #regulons_df <- import_omnipath_interactions(select_organism = 9606)
  ...
  for (ii in 1: length(tfList)){
    ...
    final_input <- NULL
    final_input <- ifelse(str_detect(names(tfList)[ii],"NOTisDeleterious"),FALSE,TRUE)
  ...
  for (i in 1:  range_opt) {
   ...
   try(
    ...
    if (is.null(net_base) == FALSE) {
      ...
      visSave(sp_g_6, file = paste0(paste0(carnival_path, "/", names(tfList)[i]),"/GRAPH",".html"), selfcontained = TRUE, background = "white")
      ####################################################
